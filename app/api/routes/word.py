...
word_use_cases = WordUseCases(word_repository)

@router.post("/words/")
async def create_word_endpoint(word: Word):
    try:
        created_word = word_use_cases.create_word(word, word_repository)
        return created_word
...
async def get_word_endpoint(word_id: int):
    try:
        word = word_use_cases.get_word(word_id, word_repository)
        if word is None:
